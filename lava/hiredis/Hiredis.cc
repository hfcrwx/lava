...
Hiredis::Hiredis(EventLoop* loop, const InetAddress& serverAddr)
    : loop_(loop), serverAddr_(serverAddr), context_(NULL) {}
...
void Hiredis::connect() {
  ...
  context_ =
      ::redisAsyncConnect(serverAddr_.toIp().c_str(), serverAddr_.port());
...
void Hiredis::connectCallback(int status) {
  if (status != REDIS_OK) {
    LOG_ERROR << context_->errstr << " failed to connect to "
